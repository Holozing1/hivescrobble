...
const KIND_PATTERNS = {
	...
	episode:
		/\b(TV series|television series|miniseries|web series|sitcom|drama series|anime series)\b/i,
} as const;
...
async function opensearch(query: string, limit = 8): Promise<string[]> {
	...
	const data = await jsonFetch<[string, string[], string[], string[]]>(
		`${API}?${params.toString()}`,
	);
...
async function batchedQuery(titles: string[]): Promise<BatchedPage[]> {
	...
	const data = await jsonFetch<{ query?: { pages?: BatchedPage[] } }>(
		`${API}?${params.toString()}`,
	);
...
export async function getSummary(
	...
	const data = await jsonFetch<SummaryResponse>(
		`${REST}/page/summary/${safe}`,
	);
	if (!data) {
		return null;
	}
...
export async function getImdbId(pageTitle: string): Promise<string | null> {
	...
	const data = await jsonFetch<WikidataResponse>(
		`${WIKIDATA_API}?${params.toString()}`,
	);
