...
function ScrobblerDisplay(props: { label: ScrobblerLabel }) {
	...
	// sessionID (so the scrobbler still binds), so the Hive block keys on a
	// non-empty sessionID. The Hive and Google sections render independently,
	// so a graduated user (Hive account + linked Google) sees BOTH.
	...
	return (
		<div role="group" aria-label={scrobbler()?.getLabel()}>
			...
			{/* Google (guest) section — renders independently of Hive, so a
			    graduated user who has both sees both. */}
			<Show when={guestAuth()}>
				<p>
					{hiveSession()
						? 'Also linked with Google'
